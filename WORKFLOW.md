...
**Version 2.3.0** — tracks the release in [`VERSION`](VERSION) (source of truth) and [`CHANGELOG.md`](CHANGELOG.md).
...
`/dev` is delegation-first (`orchestrator.md > Delegation-first`), but that bias used to be an invisible *runtime* decision — a phase either fanned out or it didn't, with nothing for the user to steer up front and no record afterward (the "I didn't see fanout come help" gap). The **Fanout plan** makes it explicit and gated, the sibling of the per-task phase plan above: `lead` declares the planned fanout strategy in a `## Fanout plan` block in `plan.md`, the gate surfaces it for sign-off, and `state.json` records both the gated intent and the runtime outcome.
