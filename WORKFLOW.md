...
**Version 2.5.1** — tracks the release in [`VERSION`](VERSION) (source of truth) and [`CHANGELOG.md`](CHANGELOG.md).
...
Parallelism is a cost the user signs off, not a default the runtime takes silently. `/dev` is single-pass-first (`orchestrator.md > Single-pass-first`); the **Fanout plan** makes that stance explicit and gated — the sibling of the per-task phase plan above. `lead` declares the planned fanout strategy (single-pass by default, fanout only where the work clears the cost bar) in a `## Fanout plan` block in `plan.md`, the gate surfaces it for sign-off, and `state.json` records both the gated intent and the runtime outcome.
...
Turn a rough intent into a signed-off spec + plan + test plan before any code is written — think first, ask only what's unspecified, and let the human gate the contract. (Steps point to the canonical subsections above for the detail.)
...
Splitting is the exception, not the reflex — one run does full-stack work. **Default:** one `/dev` run, regardless of file count, step count, or layers touched. Crossing DB + API + UI is normal full-stack work, NOT a reason to split.
