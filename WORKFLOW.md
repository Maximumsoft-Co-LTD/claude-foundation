...
**Version 2.3.2** — tracks the release in [`VERSION`](VERSION) (source of truth) and [`CHANGELOG.md`](CHANGELOG.md).
...
`/dev` is delegation-first (`orchestrator.md > Delegation-first`). The **Fanout plan** makes that bias explicit and gated, the sibling of the per-task phase plan above: `lead` declares the planned fanout strategy in a `## Fanout plan` block in `plan.md`, the gate surfaces it for sign-off, and `state.json` records both the gated intent and the runtime outcome.
