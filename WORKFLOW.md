...
Phase 1 → interview → `spec.md` (Type=feat, CRUD tasks, single user, no auth, localStorage) → `lead` proposes Vite + React + TS scaffold → `plan.md` (10 steps) → `qa` writes `test-plan.md` (each AC → unit/e2e level + assertions) → **gate** (spec ACs + plan scaffold + test plan presented; will run 1-2-3-4-5-7-8-9-10; skip 6 — no sensitive paths) → `approve`.
...
Phase 1 → interview → `spec.md` (Type=fix, reproduction = "log in from /admin, lands back on /login") → `lead` writes `plan.md` with step 1 = "add failing test `auth.spec.ts:redirect-loop`" and step 2 = "fix the redirect" → `qa` writes `test-plan.md` (regression contract: which test, how it's confirmed to fail on pre-fix code) → **gate** (will run 1-2-3-4-5-6-7-9-10; security review on — diff touches auth; skip 8 — docs not in scope) → `approve`.
