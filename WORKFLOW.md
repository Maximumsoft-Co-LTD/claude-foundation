...
| 5. Review | ✓ | ✓ | ✓ | ✓ | ✓ | light |
| 6. Security review | trigger-based | trigger-based | trigger-based | skip | skip | skip |
...
1. **Interview** — `pm` reads `FOLLOWUPS.md` first, then uses `AskUserQuestion` (≤4 questions, one batch) to capture: goal, users, scope, non-goals, constraints, **Type**, `Ship as`, and whether any open follow-up is now in scope. For `fix`, also asks for a concrete reproduction.
2. **Spec** — `pm` writes `spec.md` from the answers, including the `Type` slot and (for `fix`) a Reproduction section.
3. **Plan** — `lead` (plan mode) reads `spec.md`, runs the scope check, then:
...
4. **Gate** — `orchestrator` shows `spec.md` summary + `plan.md` outline (or epic slices) + the type-aware phase list ("will run: 1-2-3-4-5-7-9-10; skipping 6,8 — type=fix, no sensitive paths"). Wait for `approve` / `revise <notes>` / `swap <n>` (epic only). On `revise`, loop to step 2 with notes.

## Phase 2 — Implementation (autonomous after approval)

5. **Implement** — `engineer` executes `plan.md` step by step using `TaskCreate` to track progress. Marks each task done as it lands. **For `fix` runs, the very first task must be reproducing the bug via a failing test** (engineer writes it; `qa` will own it from phase 7). Before signalling done, engineer ticks each `spec.md > Acceptance criteria` checkbox or files a blocking note explaining why one cannot be ticked.
6. **Review** — `lead` (review mode) reads the diff against `plan.md` AND the `spec.md` acceptance criteria. Writes `review.md`. Blocking issues → `engineer` fixes → re-review (max 2 cycles before escalation).
7. **Security review** — *trigger-based*. If the diff matches the security-trigger list, `orchestrator` spawns `lead` in security mode. Writes `security.md`. Findings of severity `high` are blocking; `medium` and below are non-blocking and carry into `retro.md`.
8. **Test** — `qa` writes/runs unit + integration + e2e, records in `tests.md`. For `fix`, the regression test from step 5 MUST fail on the pre-fix code (qa verifies via `git stash` or equivalent) and pass on the current code. For `refactor`, qa runs the existing suite and adds tests only for behaviours that weren't already covered. Acceptance criteria are mapped to specific tests in `tests.md`. Failing tests block step 9 (max 3 fix-retry cycles before escalation). Skipped entirely for `chore` / `docs` / `spike` — qa writes a one-line stub in `tests.md` saying so.
9. **Docs touch-up** — `engineer` updates inline comments where the *why* is non-obvious and any user-facing docs the change actually touches. No new docs unless the spec asked.
10. **Ship** — `engineer` in ship mode stages the changed files, writes a commit message referencing the run ID + spec goal, and (if the repo has a remote and the user opted in at the gate) opens a PR. The commit hash + PR URL are recorded in `retro.md`. Skipped for `spike` unless the user explicitly asks to commit the exploration.
11. **Retro** — `retro` reads `plan.md` + `review.md` + `security.md` (if any) + `tests.md` + diff + commit, writes `retro.md`. Appends any new follow-ups to the shared `FOLLOWUPS.md` and marks any consumed ones closed. Surfaces *memory candidates* (facts) and *skill candidates* (procedures) for user confirmation. The orchestrator then asks the user which skill candidates to actually create and spawns `skill-creator` for each approved item.
12. **Done** — `orchestrator` prints summary: artifacts written, files changed, commit hash, PR URL, open follow-ups, skill candidates created.
...
| `pm` | 1, 2 | intent + interview answers + FOLLOWUPS | `spec.md` | `AskUserQuestion`, `Read`, `Write` |
| `lead` | 3 (plan), 6 (review), 7 (security) | `spec.md`, codebase, diff | `plan.md` / `epic.md`, `review.md`, `security.md` | `Read`, `Grep`, `LSP`, `Write`, `Edit` |
| `engineer` | 5 (implement), 9 (docs), 10 (ship) | `plan.md`, `spec.md`, diff | source, inline comments, commit, PR | `Read`, `Edit`, `Write`, `Bash`, `LSP`, `TaskCreate` |
| `qa` | 8 | `plan.md`, `spec.md`, source, diff | tests + `tests.md` | `Read`, `Write`, `Bash`, `LSP` |
| `retro` | 11 | all artifacts + diff + existing memory/skills + FOLLOWUPS | `retro.md`, INDEX status, FOLLOWUPS append | `Read`, `Write`, `Edit`, `Bash` |
...
Phase 1 → interview → `spec.md` (Type=fix, reproduction = "log in from /admin, lands back on /login") → `lead` writes `plan.md` with step 1 = "add failing test `auth.spec.ts:redirect-loop`" and step 2 = "fix the redirect" → **gate** (will run 1-2-3-4-5-6-7-9-10; security review on, touches auth) → `approve`.
...
Phase 1 → interview → `spec.md` (Type=spike, timebox=1 day, deliverable=recommendation) → `lead` writes exploration plan → **gate** (will run 1-2-3-4-5-11; skip 6-7-8-9-10 unless user opts to commit) → `approve`.
...
- User says `revise` at the gate → back to step 2.
