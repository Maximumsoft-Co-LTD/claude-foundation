...
test("deck navigation updates progress, controls, dots and slide events", async () => {
  const window = fixture();
  const { initDeck } = await import(`../src/deck.js?test=${Date.now()}`);
  const changes = [];
  ...
  assert.equal(document.getElementById("progress-bar").style.width, "66.66666666666666%");
...
test("help overlay and editable targets handle keyboard input safely", async () => {
  const window = fixture();
