...
Users keep seven intent-level commands: `/investigate`, `/change`, `/build`,
`/prove`, `/land`, optional `/deliver`, and `/dev`. The agent compiles one semantic draft and drives
...
Change Loop deliberately splits responsibility so that no single party can both do the work and certify it.
...
:::note[Version]
These docs track Change Loop **v3.5.22** — runtime API 41, provider protocol 13. Receipts recorded by earlier versions read as `provider-version-stale` and must be re-proven.
:::
