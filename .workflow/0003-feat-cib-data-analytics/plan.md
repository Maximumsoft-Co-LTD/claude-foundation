...
| Multi-file node-merge tiebreaker is ambiguous when two files give the same node id different attributes | med | Tiebreaker rule encoded in `MergeGraphs` (step 15): attributes from the file with the lower `uploaded_at` win; ties broken by file id lexicographic. Documented in package doc; unit-tested in step 15. |
