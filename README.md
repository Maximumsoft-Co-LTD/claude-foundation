...
| 7½ | **Improve** | `engineer` | *Brownfield `feat`/`fix` only* — with the suite green, a **bounded, behaviour-preserving** cleanup of *only the code this run changed* (the "improve" of understand → lock → change → improve) — bounded to the spec/plan-approved scope, **not the engineer's discretion** — committed type-aware (separate `[improve]` commit for `fix`, folded into the ship commit for `feat`). A `refactor` skips it (it *was* the improvement); greenfield skips it (right shape the first time). Doesn't re-run review/test (security only if a sink is touched); overflow becomes a follow-up. |
