...
| 2½ | **Test plan** | inline in the orchestrator (XS/S), `lead` combined mode (M) or `qa` (L) | *feat/fix/refactor only* — writes `test-plan.md` before any code: which test level proves each acceptance criterion, the edge cases to probe, what's out of test scope, and the fixtures a run needs. Signed off at the gate; `qa` executes it at phase 5. |
