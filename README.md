...
2. **Plan** — `lead` reads the spec, requests plan fanout only for S/M/L existing-code work, then synthesises parallel codebase exploration + best-practice research into `plan.md` with `file:line` references, risks, verification, and (when relevant) a rollback section. For `fix`, plan step 1 is "write failing regression test"; for `refactor`, includes a behavior-equivalence statement.
