...
10. **Implement.** Spawn `engineer` implement mode. Pass `Type`; point at `spec.md > Acceptance criteria` (it reads+ticks there). INDEX → `building`. State: `step=implement`. Resume guard, diff check, `FANOUT_REQUESTED: implement` → `phase-2-guards.md` step 10.
