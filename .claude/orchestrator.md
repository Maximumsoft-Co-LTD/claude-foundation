...
| Implement / Test / Review | 10 / 11 / 12 | 4 / 5 / 6 |
| Security | 13 | 7 |
...
**Field (greenfield|brownfield, one-way).** Orthogonal to size; classify at digest, record `field`. **Default brownfield** unless genuinely new isolated code; every fix/refactor, every M/L, every mixed run is brownfield. Brownfield turns on **understand** (current-state map) and **lock** (characterization baseline). `FIELD_UPGRADE: brownfield — <reason>` → set `field=brownfield`, run remaining brownfield, **backfill** understand/lock: re-spawn `lead` plan-revise for `plan.md > Current state`; for `test-plan.md > Baseline` — M/L the qa spawn writes it, XS/S you add the row inline. Only ratchets greenfield → brownfield.

## Phase 1 — Requirements

6. **Interview (you run it** — sub-agents can't `AskUserQuestion`). Ingest prior conversation into a **requirements digest**, estimate `size` + classify `field`, fetch reference URLs, run spec-prep fanout (opt-in), pick the UNSPECIFIED slots **ordered by the design tree** (load-bearing decisions first; defer dependents to a later batch), run the three mandatory detections (NFR / e2e-visual opt-in / error-boundary), `AskUserQuestion` one batch of 3–4 — **each choice leads with a `(Recommended)` option** — + a free-text catch-all, then save the bundle for `pm`. Sub-step procedure → **`references/interview.md`**.
...
Step *decisions* are inline; deep scan/guard mechanics (resume guard, changed-repo-set computation, security sink lists, row-by-row test detail) → **`references/phase-2-guards.md`** — read at the step it names.
...
11. **Test.** **Test runs before review** — reviewers judge code that already passes its suite. **Phase-plan guard:** `phase_plan.test == "skip"` → one-line `tests.md` stub, record `skipped_steps`, go to 12. Else: feat/refactor/fix → spawn `qa` execute mode (pass `test-plan.md` + `e2e_visual`), INDEX → `testing`, State: `step=test, next_step=review`; failing → `cycles.test++`, route to `engineer` (then re-enter test) while `cycles.test` < 3, ≥3 → escalate. chore/docs → `tests.md > Skipped` stub yourself (`next_step=review`); spike → skip test entirely (`recommendations.md` is the deliverable; go to 12 for the light review). Execution detail (visual pass, plan-contradiction/edge-case/coverage handling) → `phase-2-guards.md` step 11.
12. **Review.** **Phase-plan guard:** `phase_plan.review == "skip"` → record `skipped_steps`, go to 13. chore/docs at XS with `phase_plan.review != "run"` → skip too. Else compute the **changed-repo set**, spawn `lead` review mode (model: `sonnet` default; keep opus for `Size=L` / `## API/event contracts` / test-**infra**), INDEX → `review`, State: `step=review, next_step=security`. **Dedup:** test (step 11) runs first, so if it fanned out `team-pr-test-analyzer`, pass its `tests.md` coverage findings so the review test-coverage lens folds them in instead of re-deriving. Multi-repo → surface fanout (`references/surface-fanout.md`); `FANOUT_REQUESTED: review` → tiered workers per `## Fanout dispatch`. Verdict `fix-required` + `cycles.review` < 2 → `cycles.review++`, route to `engineer`, then **re-enter test (step 11)** so the fix is re-validated before re-review; ≥2 → escalate. Details → `phase-2-guards.md` step 12.
13. **Security review (trigger-based).** Scan every changed repo by path category + content-pattern sink (also on user request); any sensitive path → set `security_triggered=true`, spawn `lead` security mode (multi-repo → surface fanout). `fix-required` `high` → blocking: `cycles.review++` (>2 escalate), route to `engineer`, then **re-enter test (step 11) → review (12) → security (13)** so the fix is re-tested and re-reviewed; `medium`/`low` → append `FOLLOWUPS.md > Open` (`F-<run-id>-NN`, `security`, `path:line`), proceed to docs (14). Sink lists + carve-out → `phase-2-guards.md` step 13.
...
Review max 2, Test max 3. The two counters are independent and bump **only on a real fix-routing**: `cycles.test` on a failing test sent to `engineer`, `cycles.review` on a review/security fix sent to `engineer`. A review/security fix re-enters test (step 11) before re-review so test stays the final pre-ship gate — that re-validation run does **not** bump `cycles.test` unless it itself fails. At the limit → `AskUserQuestion` (continue / hand off / abort). Don't silently iterate past limits or downgrade severity to fit.
...
- Never *silently* skip a matrix-`✓` phase — the only sanctioned skip is a gate-confirmed deviation on a discretionary phase (5 Test / 6 Review / 8 Docs), recorded in `phase_plan` + `skipped_steps`. The protected set (interview, plan, gate, security-trigger, retro) is never skippable.
