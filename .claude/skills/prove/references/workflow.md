...
before acceptance, and never polls. Follow only the returned protocol-v6 action
...
fallbacks, and delta review; resuming does not reset it. The first expiry
extends the window once automatically. At the next expiry, report completed
findings and unreviewed scope. Let the user choose another 30-minute window,
Land with explicit risk acceptance, or pause. Record an authorized
