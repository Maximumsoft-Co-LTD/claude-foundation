...
description: Apply the high-leverage fundamentals that govern any non-trivial system-level architecture work — service and module boundaries, single-owner data, sync vs async interactions, resilience (timeouts, retries, circuit breakers, bulkheads), eventual vs strong consistency, observability (logs/metrics/traces, SLIs/SLOs), and backwards-compatible contract evolution. Use this skill BEFORE designing a new system, splitting or merging services, adding a new cross-service call, designing an API or event schema, planning failure modes, sizing for scale, or making any decision about how multiple services, modules, processes, or teams relate to each other. Triggers on requests like "design a system", "design the architecture", "architect this", "split this into services", "monolith vs microservices", "should this be its own service", "how should X and Y talk to each other", "sync or async", "REST or events", "circuit breaker", "timeouts", "retries", "bulkhead", "graceful degradation", "observability", "tracing", "SLO", "SLI", "error budget", "consistency model", "eventual consistency", "service boundaries", "bounded context", "Conway's law", "API design", "API versioning", "event schema", "contract evolution", "scale this system", "distributed system", "cross-service", "service mesh", "saga", "outbox", "extract a service", "strangler fig". Use it even when the user does not name a principle — the trigger is any system-level architectural work, anything that spans more than one module, service, or process boundary. The skill provides a 7-principle pre-flight checklist with worked examples, plus reference files on boundaries, communication, resilience, and observability. Skip only for purely within-one-service code work (covered by [[hexagonal-backend]]), throwaway prototypes meant to be deleted, or pure code-level refactors with no cross-boundary concerns.
---
...
- Within a bounded context, prefer the **smallest deployment unit that lets the team own a coherent piece of business value**. A new service is a permanent operational cost — separate deploy, separate monitoring, separate on-call story, a network call where a function call used to be. Pay that cost only when the boundary buys you genuine independence (independent scale, independent deploy cadence, independent failure domain) that a module inside a monolith couldn't.
...
- **Circuit breakers** around dependencies that can become flaky. After N failures in a window, fail-fast for a cool-down period instead of stacking hung calls; periodically half-open to test recovery. (Resilience4j, Polly, Hystrix patterns, or the language's equivalent.)
...
- For multi-step workflows that must succeed-or-undo across components, use **sagas with compensating actions**, not distributed transactions. Two-phase commit and XA across heterogeneous systems are usually the wrong tool — they couple availability and rarely deliver what they promise.
...
- **Three pillars, all on, structured.**
  ...
  - **Traces:** OpenTelemetry or equivalent. Trace context propagates across every component hop — HTTP headers, message metadata, DB context — so one user request produces one trace, end-to-end.
