...
description: Apply refactoring fundamentals — change structure without changing behavior, wear one hat at a time, get green or characterize before you touch, take small reversible steps, let smells and upcoming change drive it, know when NOT to refactor, keep large refactorings shippable (Mikado / strangler). Use BEFORE reshaping existing, working code whose behavior should stay identical and only the shape is wrong: break up an oversized function/class/file, extract or move code, rename, consolidate copy-pasted logic into one place, replace a switch/type-code repeated across files with polymorphism, untangle code that reaches into another object's internals, restructure legacy/untested code (characterize first), or migrate a library/ORM/data layer across many call sites without breaking callers. The ask often sounds like "clean this up / break this into smaller pieces / extract / split / pull into one place / de-duplicate / untangle / is there a cleaner way / it works but it's a mess / pay down tech debt" — even when the word "refactor" never appears. This is the safe-transformation process that moves code toward the shape [[programming-fundamentals]] defines. Includes references on code smells, the refactoring catalog, characterization tests, and large-scale work. Skip greenfield code, new features, bug fixes, reformatting/prettier, deleting code, and explaining what code does.
---
