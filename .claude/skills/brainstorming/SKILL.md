...
**Record the decision trail — don't let it evaporate once the user picks.** Capture the chosen option + the rejected alternatives + the one-line why; that's the ADR the plan already carries in `Approach` / `Alternatives considered` / `Hard-to-reverse decisions` (no new artifact — feed those sections). Likewise, any domain term that surfaced and needed defining goes to the spec `Glossary` ([[ddd-strategic]] principle 3) so spec/plan/code use it verbatim. This is the grill-with-docs discipline: the grilling produces the docs as a by-product, it doesn't bolt them on after.
