...
- `references/shared-state-and-async.md` — escape hatches (immutability, confinement, message-passing); lock vs atomic vs CAS vs optimistic-version decision rules; deadlock recipes; async/await pitfalls; bounded-concurrency patterns (semaphore, worker pool, backpressure).
