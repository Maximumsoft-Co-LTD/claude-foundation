---
name: ddd-strategic
description: Apply the strategic half of Domain-Driven Design — the parts that decide WHERE the model lives and WHAT language it speaks, not how its code is layered. Covers subdomain classification (core/supporting/generic) so you know whether DDD pays at all, bounded contexts as linguistic + model boundaries (one word, one meaning per context), ubiquitous language as a per-context naming discipline, the seven context-mapping patterns (shared kernel, customer/supplier, conformist, anticorruption layer, open host service, published language, separate ways), Event Storming and Domain Storytelling as discovery workshops with concrete code-mapping paths, the one tactical piece not covered by sibling skills — aggregate sizing via Vernon's four rules, and the separation of internal domain events from cross-context integration events. Use this skill BEFORE drawing service boundaries on a system with real business complexity, naming a new concept that lives in multiple contexts, integrating with another team's system (upstream or downstream), deciding how much to invest in a piece of the domain, sizing an aggregate, running a discovery workshop, or diagnosing a model that has gone wrong (anemic, leaky, oversized, missing language). Triggers on requests like "DDD", "domain-driven design", "bounded context", "ubiquitous language", "context map", "subdomain", "core domain", "supporting domain", "generic subdomain", "event storming", "domain storytelling", "anticorruption layer", "ACL", "shared kernel", "conformist", "open host service", "published language", "aggregate root", "aggregate boundary", "anemic domain model", "domain event vs integration event", or any moment a feature spans two contexts that use the same word for different things. Use it even when the user does not name DDD explicitly — the trigger is *strategic* design work (where the boundaries go, what language they speak, how they relate) on a domain with real business rules. Skip for generic CRUD admin tools, throwaway prototypes, single-context features with no cross-boundary concerns, and pure code-level work (covered by [[programming-fundamentals]] and [[hexagonal-backend]]).
---
...
- **Rename ruthlessly.** When the glossary says the word is `policy`, rename `record`, `entry`, `agreement`, and `contract` to `policy` across the codebase, the DB schema, the API, the dashboards, and the runbooks. The translation cost is paid once at rename time and saved forever after.
