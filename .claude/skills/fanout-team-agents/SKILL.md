...
| Phase 1 step 2 — plan | `lead` (Mode A) | **single-pass** | ≥ 2 independently-researchable integration points whose research is genuinely substantial; dispatch both codebase and best-practice workers per point (single-pass for XS/S, pure-greenfield, one point, or a handful of quick reads) |
...
- **Coordinator nests `general-purpose` helpers (foreground, no Case 3, no background)** — the orchestrator spawns **one** foreground `lead`/`qa` (its **Surface-coordinator variant**); that coordinator **nests one per-repo helper per repo itself** as `subagent_type="general-purpose"` (the agent's Per-repo contract inlined, `repo_root=<r>`, foreground, one message), then synthesises in the **same** spawn. `general-purpose` is deliberate: a *foreground* same-message batch of the 5 /dev-worker types (`pm|lead|engineer|qa|retro`) self-blocks on the 2nd spawn under `dev-agent-guard.sh` Case 3 (the first return touches `.last_worker_return` before the next pre-check), but `general-purpose` falls through Case 3 — exactly like the lens axis's `team-*`. This **replaces** the old orchestrator-direct `run_in_background` batch, which dodged Case 3 (background launches don't touch the marker) but at two costs now retired — whether the harness reliably launches N background agents from one message was inferred-never-verified, and background state-discipline isn't hook-enforced. Width is now handled *inside* the coordinator via Case-3-free nesting (possible because `lead`/`qa` hold `Agent`, v2.1.172+). Full mechanism + provenance: `orchestrator.md > Surface (per-repo) fanout > Dispatch`.
...
- **Mis-judging whether the work admits fanout — in either direction.** Single-pass-first (`.claude/orchestrator.md > Single-pass-first`) means a single pass is the default and fanout is the justified exception. Under this default the **more likely error is over-fanning-out**: firing six workers (or N prep probes) on a diff/research surface a single pass would have finished before the helpers even spun up — paying coordination + cold-start + synthesis for nothing, and producing a larger artifact. The **opposite** error is real but rarer: **staying single-pass when large, independent, disjoint domains genuinely exist** leaves real parallel wall-clock on the table. Fan out only when a phase clears its eligibility bar AND none of the `Don't use when` guardrails apply AND the parallel work is substantial enough to repay the cost.
