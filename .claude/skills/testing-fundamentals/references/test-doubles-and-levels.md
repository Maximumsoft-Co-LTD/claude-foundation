...
- **Fake** — a working but lightweight implementation of the real thing (an in-memory repository, an in-memory payment gateway, SQLite standing in for Postgres). Behaves like the real dependency, so tests exercise real logic against it. Often the best double when many tests need the same collaborator.
