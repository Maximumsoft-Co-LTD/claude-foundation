...
description: Apply testing fundamentals — test behaviour not implementation, the test pyramid, what to test (contract, boundaries, error paths, the bug you just fixed), test doubles with discipline, isolated/deterministic/fast tests, arrange-act-assert with behaviour names, coverage as a flashlight. Use BEFORE writing a test, deciding what to test, designing a suite, choosing test levels (unit/integration/e2e), or reviewing coverage; the trigger is any "how should this be tested" decision, even when no principle is named. Includes references on test design and on test doubles & levels. Skip throwaway scripts, chore/docs/spike work, and trivial config.
---
...
This skill is a **pre-flight**: read it before you write the test, choose the level, or design the suite. It owns test *strategy and design* — what deserves a test, at which level, with which doubles, asserting what. It is the design-time companion to the execution the `/dev` `qa` agent does at phase 7 (`qa` runs the suite, maps every acceptance criterion to a test, records results in `tests.md`); this skill is the "how to test well" that `qa` and `engineer` reach for, not a copy of `qa`'s procedure.
...
- If you set a CI threshold, treat it as a ratchet that catches *drops* on changed code, not a bar that justifies testing trivia to clear it.
...
This skill is design-time. In `/dev`, the `qa` agent (phase 7) *executes* the suite and records results in `tests.md`, and the `engineer` writes tests as it implements — both reach for these fundamentals to decide *what good looks like*; this skill does not duplicate `qa`'s run/record procedure. Two type-specific contracts hand off to neighbours: for a **fix**, the regression test comes first and must fail on the old code (get the repro via [[debug-fundamentals]]); for a **refactor**, the baseline is a **characterization test** owned by [[refactoring-fundamentals]] — capture it before the structural change, then this skill governs any *new* tests you add.
