...
description: Generate a standard onboarding documentation suite for an EXISTING (brownfield) codebase — OVERVIEW.md, ARCHITECTURE.md, TECHSTACK.md, DATAMODEL.md, COREFEATURE.md (with mermaid sequence diagrams), API.md, and a self-contained document.html viewer that renders them all. Use whenever the user wants to document, map, or onboard an existing project — phrasings like "document this codebase", "init docs for this repo", "generate project documentation", "create ARCHITECTURE.md / DATAMODEL.md", "I need an overview of this system", "write onboarding docs", "ทำเอกสารโปรเจกต์", "สร้าง docs ให้โปรเจกต์นี้", "อธิบายสถาปัตยกรรมของโปรเจกต์" — even when they don't list every file. This documents code that already exists by reading it; it is NOT for a greenfield project with no code yet.
---
...
- A doc's domain genuinely **doesn't exist** (a CLI with no HTTP API, a stateless service with no datastore). Don't pad it — write a one-line "Not applicable: this project has no persistent datastore" and move on. An honest stub beats a fabricated schema. (You may also drop the file entirely if the user prefers; the viewer adapts to whatever `.md` files are present.)
...
- `sequenceDiagram` (COREFEATURE), `erDiagram` (DATAMODEL), `flowchart TD`/`graph LR` (ARCHITECTURE).
...
- **Diagrams render**: COREFEATURE has a sequence diagram per flow; DATAMODEL has an ER diagram; ARCHITECTURE has a component/flow diagram; all valid mermaid.
...
- `plan-writing` / `brainstorming` own *forward-looking* design docs for code that doesn't exist yet; this skill documents code that does.
