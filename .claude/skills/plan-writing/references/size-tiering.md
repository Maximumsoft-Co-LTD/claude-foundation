...
This file is the picker. Lead sets `Size` in the plan's frontmatter before drafting `Steps`.
...
"Real logic" = branching, state, side effects to design. "Simple logic" = one branch, no state. "Contract / schema" = public API surface, DB schema, queue message shape, IPC, event payload.
...
3. **Is there real logic to design?** (branching, state machine, retry policy, ordering decision, concurrency)
   → **M** if single-subsystem, **L** if multi-subsystem (already caught in step 2).
...
- **One-file change that adds branching, state, or retry policy** → at least S, often M. The complexity is in the logic, not the file spread.
...
- **Many-file change that's "ripple from removing one thing"** → S or M depending on logic in the call sites. Removal alone isn't M; design of the removal is.
- **Touches a security-sensitive path** (auth, crypto, exec, deserialise) → bump up one tier; security review will fire anyway and the plan benefits from more documentation.
...
The logic density wins over file count. **M.** A state machine deserves a diagram, AC tags per state transition, and observability around state changes — all M-tier sections.
...
Rough budget (for lead, with construction skill already loaded):
...
| L | 1–2 hr | Two diagrams + Dependencies + L-grade Rollback |
