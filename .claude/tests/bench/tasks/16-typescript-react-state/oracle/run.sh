...
if [ -n "$tests" ] && run_suite; then
  cp "$SEED/src/panel-state.js" "$WORK/src/panel-state.js"
...
let raw="";process.stdin.on("data",d=>raw+=d).on("end",()=>{let o={results:{}};
try{o=JSON.parse(raw)}catch{};o.results.CASE_REGRESSION_TEST=process.env.REGRESSION;
