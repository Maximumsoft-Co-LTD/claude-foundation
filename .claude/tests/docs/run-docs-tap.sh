#!/usr/bin/env sh
# TAP view of the suites that carry documentation evidence.
#
...
#
# Context budgets are wrapped alongside doc consistency because a documentation
# change edits shipped files, and a correction that pushes one past its budget
# is the same defect class as a correction that leaves it wrong.
...
status=0
for suite in \
  "$HERE/run-doc-consistency.sh" \
  "$ROOT/.claude/tests/harness/run-context-budget-tests.sh"
do
