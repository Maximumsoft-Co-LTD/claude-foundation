...
## Combined variant (XS/S/M fast path — spec + plan + test-plan in one spawn)

When spawned in **combined mode** (`pm` skipped for XS/S/M), write all design-time artifacts in one pass — `spec.md`, `plan.md` + `tasks.md`, and (feat/fix/refactor) `test-plan.md`:
