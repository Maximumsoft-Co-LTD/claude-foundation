...
2. **Re-derive size yourself — never anchor on the orchestrator's estimate** (made from the digest, pre-code-walk). Hard tripwires that mean NOT XS: >~2 files, ANY persisted-data/storage-key/schema/API-contract change (**a data migration is never XS**), or a blast radius needing current-state mapping. Tier exceeds estimate → STOP, return `SIZE_UPGRADE: <size> — <reason>` first line (your `spec.md` stands). **Re-derive `field` the same way:** an estimated-greenfield run editing/wiring into existing code → `FIELD_UPGRADE: brownfield — <reason>` first line (the fast path stays, but the plan gains a `Current state` note + the lock baseline); else record the resolved field in the `plan.md` `**Field**:` slot. Otherwise write `plan.md` per Mode A at XS/S compactness.
