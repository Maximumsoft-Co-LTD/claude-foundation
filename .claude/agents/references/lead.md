...
**Model:** use the frontmatter `sonnet` default. The orchestrator may explicitly override it only at the high-stakes cases in `orchestrator/references/model-tiers.md`; `.spawn_log` records requested versus pinned tier.

## Current state (Mode A — scoped current-state map)

Required for **brownfield** (by `field`, not Type/Size), but always bounded to the changed surface. Size changes how much design detail is recorded, not whether the agent re-walks the repository. Skip for greenfield isolated new files, `chore`/`docs` not touching live code, `spike`.

If the prompt has **`context.md`** or prep findings, **synthesise them** — re-cite each `path#anchor`, spot-check load-bearing claims, and LSP-walk only a named gap. **`plan.md > ## Current state` must NOT duplicate `context.md` — point, don't paste:** open with `> Full map: context.md > ## Current state` and write only this change's overlay. With no map, inspect the minimum entry point + blast radius needed; a broader walk requires `exec_reason: material-context-gap`. The engineer reads the same map, so the pointer keeps one source of truth. **Evidence, not authority** — an unre-resolvable claim is a finding, not a fact. For points you walk, use **LSP find-references + go-to-definition**, not memory:
...
- **An invariant may say no more than its evidence.** State the actual constraint at `path#anchor` and label any inference. A guardrail unsupported by evidence is blocking; omit an uncertain constraint and return `CONTEXT_GAP` instead.
...
## Combined variant (one warm design executor, any size)

This section is the executor-neutral contract. The orchestrator may follow it inline, fork it from warm context, or pass it to one proof-gated lead at any size. Reuse the digest, Q&A, context map, and size/field call already supplied; a tier upgrade changes depth and the spawn ceiling, not automatically the executor. `BLOCKER:` stops the run. Write all design-time artifacts in one pass — `spec.md`, `plan.md` + `tasks.md`, and (feat/fix/refactor) `test-plan.md`:
...
## Recruit help when fanout is authorized (direct nesting — all modes)

You hold `Agent`, but may use it only when the parent prompt carries `fanout_authorized: true`, names the parallel-payoff/independence proof, and provides disjoint child scopes. Without all three, stay single-pass regardless of size or `## Fanout plan`; return `BLOCKER:` only when the missing work truly prevents the artifact. When authorized, integrate returns and remain sole writer. Follow the registry path in `.claude/skills/fanout-team-agents/SKILL.md`.
...
Review itself may be cold-spawned for independent judgment, but nested review fanout still needs separate `fanout_authorized: true` proof:
- Single coherent diff → direct review at every size.
- Multiple substantial disjoint surfaces → core lenses may run in parallel when authorized.
- Public-contract/type-shape/high-stakes work → add `team-type-design-analyzer` only when that lens has a distinct scoped input.

Size sets adversarial depth and the maximum lens budget; it never proves nested fanout. Comment-accuracy and simplification remain lenses inside `team-code-reviewer`.
...
**Per-bucket fanout (single-pass-first).** Fan out only when the prompt carries `fanout_authorized: true`, the diff trips ≥2 independent buckets, and per-bucket work is substantial. Otherwise stay single-pass.
