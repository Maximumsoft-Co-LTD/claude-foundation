...
**The frontmatter pins `sonnet`, so "default" means what actually happens when the orchestrator passes no `model` param** — opus is the explicit `model="opus"` override at the escalation points above (`lead` is the worker guard Case 4 lets vary). It pinned **opus** until 2026-07-28, which made the documented default the one you had to remember to ask for: a brownfield-S benchmark recorded `lead pin:opus` — no param passed, opus bought — in **three of six otherwise identical runs**, against a playbook that says sonnet at both S spawns. Same task class, different tier, run to run. `.spawn_log` column 3 now records which governed (`req:<tier>` = an override was passed, `pin:<tier>` = the frontmatter decided), and `dev-metrics.sh` prints the tally.
...
- **An invariant may say no more than its evidence.** Citing a `path#anchor` proves the line *came from* somewhere; it does not license the conclusion drawn from it. Write what the code or comment actually constrains, and keep any inference you add visibly separate from the quote. Observed on a brownfield-S migration: `## Current state` quoted a seed comment — *"Do not change its contract"*, meaning `readAll`/`writeAll` keep their signatures — and appended *"`migrate.js` calls these functions, never touches `fs`/`FILE` directly"*, which the comment never said. That inflation reached `tasks.md > ## Guardrails`, the engineer's only up-front invariant read, and so **forbade the temp-file-and-rename the acceptance criteria required**; the run shipped an in-place overwrite with no backup and still graded a pass. A wrong guardrail is more expensive than a missing one — it is obeyed.
- **Cross-check Guardrails against the acceptance set before drafting tasks.** For each guardrail, ask which `AC#` it constrains and whether any becomes unachievable. If one does, the plan is contradictory: return `BLOCKER: <guardrail> makes <AC#> unachievable — <which is wrong>` and let the gate decide, rather than shipping tasks that cannot satisfy the contract.
...
**Existing-code research (step 9 of the original — folds into step 3/9 here).** LSP first, grep second. **Single-pass-first** (`orchestrator.md > Single-pass-first`): write `plan.md` from your own walk by default. Self-dispatch a worker per point (see *Recruit help*) **only** when **2+ integration points in disjoint surfaces** (separate modules/folders/repos) AND the research is substantial. **Disjoint surface, not raw count, is the bar.** If `context.md` (or plan-prep) already mapped current state, fan out only for residual best-practice research — never to re-map current state. Skip fanout for XS/pure-greenfield/straightforward.
...
## Combined variant (the one-pass design contract — spawned at M, followed INLINE by the orchestrator at XS/S)

This section is the contract, not a description of who executes it. At **M** you are spawned in combined mode (`pm` skipped). At **XS/S** there is no spawn at all: the orchestrator follows these same steps itself (`orchestrator/references/xs-s-fast-path.md`), because it already holds the digest, the intent, the Q&A and the size/field call that a cold spawn would spend a 29 KB brief re-deriving. Same sections, same depth, same reference access either way — only the pen moves. **On the inline path the first-line signals below have no return to ride on:** `SIZE_UPGRADE` / `FIELD_UPGRADE` means the orchestrator re-records the axis and runs the rest at the new tier (a `SIZE_UPGRADE: M` hands Design back to a cold `lead` spawn), and `BLOCKER:` means it stops as it would on a worker's return. Write all design-time artifacts in one pass — `spec.md`, `plan.md` + `tasks.md`, and (feat/fix/refactor) `test-plan.md`:
...
2. **Re-derive size yourself — never anchor on the orchestrator's estimate** (made from the digest, pre-code-walk). Hard tripwires that mean NOT XS: >~2 files, ANY persisted-data/storage-key/schema/API-contract change (**a data migration is never XS**), or a blast radius needing current-state mapping. Tier exceeds estimate → STOP, return `SIZE_UPGRADE: <size> — <reason>` first line (your `spec.md` stands). **Re-derive `field` the same way:** an estimated-greenfield run editing/wiring into existing code → `FIELD_UPGRADE: brownfield — <reason>` first line (the fast path stays, but the plan gains a `Current state` note + the lock baseline); else record the resolved field in the `plan.md` `**Field**:` slot. Otherwise write `plan.md` + `tasks.md` per Mode A — XS/S at compact depth; **M at full Mode A depth per plan `Size`** (incl. Scaffold; `context.md` in the prompt is the current-state map — spot-check, don't re-walk).
3. **XS/S: no spec-prep/plan-prep fanout** — needing to direct-nest research helpers at all is itself evidence the run isn't XS/S; return the `SIZE_UPGRADE` line. **M:** the orchestrator pre-builds `context.md` (brownfield) / prep findings and passes them; recruit residual helpers per *Recruit help* only for genuinely uncovered points.
4. **Test plan — fold it in (feat/fix/refactor only).** After `plan.md` + `tasks.md`, write `.workflow/<id>/test-plan.md` per `qa.md > Mode: Test plan` (map every acceptance scenario + its boundary/error scenario + any `measured:` target to the owning level + assertion; edge cases; fixtures + a **proactively-picked** runner in `Execution mechanism`; `fix` Regression contract / `refactor` Baseline; coverage floors; and **only when `e2e_visual=on`** — for a UI-touching diff the `Visual verification` rows + settled-render note). **Write it as an adversarial check on your plan** — is every AC verifiable, what unhappy path is unstated, is any reachable input `undefined → spec gap`, **does any `## Guardrails` line make an AC unachievable**; a reachable security/data-integrity undefined path, or a guardrail that blocks an AC → `BLOCKER:` first line. **A data-safety AC needs a test that exercises the real path, not the pure helper**: a migration graded on "the original data survives an interruption" is not covered by unit tests of its name-splitting function — the Coverage row must name the end-to-end run against a copy of the real fixture. **chore/docs/spike → no test-plan.** Phase 2 executes it — `qa` at M (or `e2e_visual=on`); at XS/S the Implement step builds the rows (inline in main, since Phase 1 drafted warm) and the orchestrator runs the suite.
...
When re-spawned with gate-revise notes (**Gate** `revise`), you patch the **existing** `plan.md`, not regenerate it:
...
- L-tier / public-contract / type-shape / high-stakes → **default the full 4** (core 3 + `team-type-design-analyzer`); direct needs a stated reason. (Comment-accuracy and simplification are lenses INSIDE `team-code-reviewer` now — the standalone `team-comment-analyzer`/`team-code-simplifier` workers were retired 2026-07-15.)

No new types skips `team-type-design-analyzer`. Fanout per-agent sections go in `review.md > Per-agent findings`; Tasks-adherence + Acceptance-criteria rows are still walked one-by-one (`WORKFLOW.md > Anti-bias rule`).
...
**Simplicity lens (single-pass reviews only — when fanout ran, `team-code-reviewer`'s Simplification lens owns it).** Scan against `coding-discipline > Simplicity First` + its decision ladder: speculative abstraction, unrequested config/flags, a hand-rolled routine the stdlib/an installed dep does, a block that could be a fraction of its size. Flag genuine over-engineering as **non-blocking** (`path:line`, name the simpler form) — blocking only if it hides a real correctness/security risk. "Could be shorter" is not a finding.

## Security fanout (Mode C step 1)

**Per-bucket fanout (single-pass-first).** Each bucket is an independent threat surface; **fan out only when the diff trips ≥ 2 buckets** AND the per-bucket work is substantial — self-dispatch one `team-code-reviewer` per bucket (see *Recruit help*) — no signal fallback; a genuine block returns `BLOCKER:` naming why. A single-bucket or quick multi-bucket diff stays single-pass.
