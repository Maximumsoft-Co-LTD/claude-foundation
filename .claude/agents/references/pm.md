...
- A note that opens a genuinely new unfillable slot → embed `[NEEDS CLARIFICATION]` rather than guessing; the orchestrator decides whether to ask one narrow question.
...
- **Registry path** (`.claude/skills/fanout-team-agents/SKILL.md`) — read `team_registry`: `live` → by name; `inline-fallback` → `general-purpose` with `.claude/agents/team-<role>.md` inlined; `unknown` → try named, fall to inline on `not found`, report the path. A miss never drops you to a single inline lookup.
...
- **Guardrails** — helpers are read-only, never write `spec.md`/`state.json`. Requirement ambiguity is NOT a probe — a genuine unknown returns as `BLOCKER:`. **One level of split:** end every helper prompt with `You are a nested helper: handle this one sub-scope directly and do NOT spawn further agents.`
