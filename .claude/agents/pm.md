...
- **Split + spawn** — one `team-codebase-explorer` per `codebase-*` fact, one `team-best-practice-researcher` per `best-practice-*` question — **N copies of the same worker, one per independent probe** (that horizontal scaling is the point, not one-of-each), **in a single message** (parallel), **cap 4**. Each helper starts fresh: give it the run id/type, the relevant spec excerpt, its exact question, and the sections to return.
- **Registry path** (full rule: `.claude/skills/fanout-team-agents/SKILL.md`) — read `team_registry`: `live` → `team-*` by name; `inline-fallback` → `general-purpose` with `.claude/agents/team-<role>.md` inlined; `unknown` → try named, fall to inline on `Agent type ... not found`, report the path used (orchestrator records it). A miss never drops you back to a single inline lookup.
