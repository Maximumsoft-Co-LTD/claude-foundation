...
- **Registry path** — read `team_registry` from your prompt: `live` → spawn `team-*` by name; `inline-fallback` → spawn `general-purpose` with the worker's role contract (`.claude/agents/team-<role>.md`) inlined; `unknown` → attempt `team-*`, fall to inline on `Agent type ... not found`, and report the path you used in your return so the orchestrator records `team_registry`. A registry miss never drops you back to a single inline lookup — only the spawn mechanism changes.
