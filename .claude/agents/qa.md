...
4b. **Visual verification pass** (UI-touching diffs only, executing `test-plan.md > Visual verification`). After the green assertion run, **reusing the same browser session your e2e tests already opened** (Playwright `page.screenshot()` / Cypress screenshot — NOT a fresh browser install; that is the 32-min Chromium cost) capture each planned viewport to a PNG, then **open each PNG with `Read`** (it renders images — that is your eye) and judge the planned visual properties. Record findings in `tests.md > Visual verification` (viewport · what you saw · pass / defect). A **real layout / readability defect** (mid-word break, overflow, overlap, clipped content) is a **blocking finding** → **set `tests.md` Status = `failing`** and route back to engineer, exactly like a failing assertion — `scrollWidth`-passing ≠ readable; a literal all-*assertions*-green run must NOT report Status = `passing` while a visual defect stands. A **cosmetic-polish nit** the spec doesn't pin is non-blocking → record in `tests.md > Edge-case gaps` for retro to lift into a follow-up. **No reusable live browser session?** — i.e. no browser-based e2e ran this suite (jsdom/happy-dom-only, or no e2e at all on a UI diff) OR no browser tooling installed: do NOT install or boot a browser just to screenshot (that re-incurs the 32-min cost) — record `visual: deferred to orchestrator MCP backstop` in `tests.md > Visual verification` and return that exact note so the orchestrator runs the check via its browser MCP.
...
- Set the single **run-level** `Cycle` counter. Carry each repo's below-floor coverage rows and edge-case gaps up so the orchestrator can escalate them once. **Carry each repo's Visual verification findings up too** — surface any blocking visual defect (→ `failing`) and any `visual: deferred to orchestrator MCP backstop` note (so the orchestrator runs the MCP backstop for that repo); a UI defect in a non-primary repo must not vanish in synthesis.
