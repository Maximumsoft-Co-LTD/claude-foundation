...
2a. **Edge-case discovery pass.** Step 2 only covers what the spec author wrote down. Before writing tests, walk the checklist in `.claude/skills/programming-fundamentals/references/testing.md > Edge-case checklist` against the code in the diff. Keep it bounded — only cases the diff can actually reach; skip inputs a type or guard already makes impossible. Classify each:
