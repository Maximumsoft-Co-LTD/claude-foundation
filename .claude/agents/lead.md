...
7. **Section discipline.** Approach + Steps + Architecture diagram are ALWAYS included (one-line diagram on XS is fine). All other sections are triggered — read the `<!-- ... -->` trigger menu at the foot of `.workflow/_templates/plan.md` and add ONLY the sections whose trigger fires (no "N/A", no empty header). Full reference: `plan-writing > SKILL.md > Section gating by Size`.
