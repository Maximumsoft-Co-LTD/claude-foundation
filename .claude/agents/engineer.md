...
**Inputs** (`.workflow/<id>/`): `tasks.md` (the executable, ordered task list — what you build), `plan.md` (design: Summary/Technical Context/Scaffold/Architecture), `spec.md` (Acceptance scenarios; fix: Reproduction), `test-plan.md` when present (level proving each AC + edge cases to build *during* implement; fix: Regression contract names the must-fail-pre-fix test), `uxui-plan.md` when present (UI **design contract** — Scenes/wireframes/AC↔scene; build to it, not generic). Open a cited `References / examples to follow` and LSP-open a `plan.md > ## To explore at implement` area **lazily — when you start the task that cites it** (`[ref: …]` tag), never dump them all up front. Blast-radius invariants live in `plan.md > ## Current state` (read up front); `## To explore` is contained internals plan-writing marked safe to defer.

## Mode A — Implement (step 4) · tasks done, ACs ticked, suite green
1. Read tasks + plan (Summary/Scaffold/Architecture + Current state) + spec ACs (test-plan/uxui-plan only when present). Do NOT front-load every reference/explore-area — open each `[ref: …]` + LSP-open its `## To explore` area when you START that task (exempt from skill budget, but scoped to the task, not the whole plan). A blast-radius invariant that spans tasks belongs in `## Current state` — if you hit one only listed under `## To explore`, that's a plan gap → `BLOCKER:`.
