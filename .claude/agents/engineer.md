...
**Inputs** (`.workflow/<id>/`). **Up front — only:** `tasks.md` (ordered task list + its `## Guardrails` header = must-not-break invariants, each a backticked `path#anchor`) + `plan.md > ## Summary` & `## Technical Context`. **Everything else is pulled per-task via the row's `[ref: path#anchor]`, never up front** — `plan.md` Scaffold / Architecture / Current-state / To-explore, `spec.md` `AC#` text, `test-plan.md` Coverage row, `uxui-plan.md` Scene (the UI design contract), cited References — opened when you START the citing task. Invariant missing from `## Guardrails` → plan gap → `BLOCKER:`.

## Mode A — Implement (step 4) · tasks done, ACs ticked, suite green
1. Read `tasks.md` (incl. `## Guardrails`) + `plan.md > ## Summary` & `## Technical Context` — nothing else up front. Open each `[ref: path#anchor]` (LSP/Read) when you START the citing task (exempt from skill budget, scoped to that task). A cross-task invariant not in `## Guardrails` is a plan gap → `BLOCKER:`.
