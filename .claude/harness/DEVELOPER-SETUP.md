...
Change Loop v3.5.8 front-loads material decisions so Build and Prove can run to a
...
2. Verify `claude-foundation version` is `3.5.8` and the repository runtime API
...
3. When the CLI itself is behind, upgrade it: `brew upgrade claude-foundation`.
   If instead the pinned source is absent, clone tag `v3.5.8` from
   `Maximumsoft-Co-LTD/claude-foundation` into
   `~/.local/share/claude-foundation/3.5.8`.
