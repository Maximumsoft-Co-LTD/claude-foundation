...
Foundation v3.3.2 front-loads material decisions so Build and Prove can run to a
...
2. Verify `claude-foundation version` is `3.3.2` and the repository runtime API
   is `24`.
3. If the pinned source is absent, clone tag `v3.3.2` from
   `Maximumsoft-Co-LTD/claude-foundation` into
   `~/.local/share/claude-foundation/3.3.2`.
