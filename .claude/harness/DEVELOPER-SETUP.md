...
Foundation v3.2.28 front-loads material decisions so Build and Prove can run to a
...
2. Verify `claude-foundation version` is `3.2.28` and the repository runtime API
   is `21`.
3. If the pinned source is absent, clone tag `v3.2.28` from
   `Maximumsoft-Co-LTD/claude-foundation` into
   `~/.local/share/claude-foundation/3.2.28`.
