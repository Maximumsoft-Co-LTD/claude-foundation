...
4. **Contract Gate (runs unless already approved).** If `state.step=gate` with `next_step=implement`, or `INDEX.md` is `approved`, continue. Otherwise fold shards/backfill, run `sh .claude/hooks/artifact-lint.sh --contract .workflow/<id>/`, then present intent and hard-to-reverse decisions for human approval. Structural checks are not repeated manually.

5. **Run Phase 2 exactly as written:** Implement → Change Gate (Impacted AC evidence + triggered semantic/security review) → Ship Gate (Full + lint/type/static) → docs/ship → Close. Do not re-derive the mechanics here. Write `state.json` after each step. A review/security fix re-enters targeted Test; the converged diff must pass Ship Gate. Surface any worker `BLOCKER:` via `AskUserQuestion`.
