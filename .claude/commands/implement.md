...
3. **Prerequisite check (you must be ready for autonomous work).**
   ...
   - **`test-plan.md` missing for `feat` / `fix` / `refactor`** → it's a gate prerequisite (the consistency scan maps every AC to a Coverage-plan row). Produce it first: run orchestrator **step 8a** here (XS → write it inline from the template; S/M/L → spawn `qa` in test-plan mode), then continue. (chore/docs/spike correctly have none.)
