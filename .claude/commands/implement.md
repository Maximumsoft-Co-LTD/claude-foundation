...
5. **Run Phase 2 — orchestrator steps 10–18, exactly as written** (implement → test → review → security → docs → ship → `done_at`+retro → skill handoff → close). **Do not re-derive them here** (step 1 — that file is the source of truth); the orchestrator owns every rule: the implement-fanout consumer contract for disjoint-phase `feat`, the review/test cycle limits (`Cycle escalation`), the trigger-gated security review, the gate-decided ship (default no → ready-to-run commit command). Write the complete `state.json` after each step (`State discipline`). Test (11) runs before review and a review/security fix re-enters it, so test stays the final pre-ship gate. Surface any worker `BLOCKER:` / "needs user input" via `AskUserQuestion`.
