...
5. **Test-plan check.** Confirm `test-plan.md` exists and has a Coverage-plan row for every `spec.md` AC (including each AC's `on error / at boundary:` clause and any `measured:` target). Re-spawn `qa` once if an AC is unmapped. (Spec-only: `[pending plan]` rows are expected, not a failure — backfilled at the gate.)
