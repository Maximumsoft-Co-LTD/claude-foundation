...
3. **Spawn `qa` in test-plan mode** (`qa.md > Mode: Test plan`). **Make `team-slice: test-plan` the first line of the spawn prompt** — it tags this as a parallel-safe Phase-1 shard producer so the state hooks scope correctly (`orchestrator.md > State discipline > Team-mode Phase-1 sharding`). Pass the run id, the `Type`, `repo_root` / `branch`, and **`e2e_visual`** (read `state.json > e2e_visual`, else `spec.md` frontmatter `E2E + visual`; treat unset as `off` — browser-based e2e/visual is opt-in) from `state.json`. Point it at `spec.md` + `plan.md` as authoritative; don't re-list the ACs inline. `qa` writes `test-plan.md` (Coverage plan per AC, Edge cases to probe, Fixtures + execution mechanism, the type-specialised Regression contract / Baseline, and — **only when `e2e_visual=on`** — the e2e level + Visual verification rows for UI diffs) and returns its summary.
...
5. **Write `state.test-plan.json` and stop.** This is a parallel-safe Phase-1 slice, so write your **own shard**, NOT the shared `state.json` cursor (`orchestrator.md > State discipline > Team-mode Phase-1 sharding`) — that keeps a concurrent `/dev-plan` or `/uxui-plan` on the same run from clobbering your write. Write `.workflow/<id>/state.test-plan.json`: `{"status":"done","last_agent":"qa","last_updated":<fresh ISO>}`. **Leave `state.json` / `INDEX.md` untouched** — the gate folds shards in. Tell the user the path and that `/implement <id>` (or `/dev --resume <id>`) will carry the run through the gate → implement → the test phase that **executes** this plan.
