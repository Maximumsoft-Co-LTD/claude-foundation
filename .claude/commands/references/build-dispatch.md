...
lease, then regenerate its `packet --task`. Give each native worker only that
packet and repository state; never replay the parent transcript. Spawn every
...
and Prove owns the aggregate graph join.
