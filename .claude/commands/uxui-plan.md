...
6. **Write your shard and stop.** Write `state.uxui.json` per [`.claude/orchestrator/references/team-mode-sharding.md`](../orchestrator/references/team-mode-sharding.md) — set `ac_covered` as progress telemetry; Contract Gate later derives the exact map from `uxui-plan.md`. Leave `state.json`/`INDEX.md` untouched. Tell the user the path + next move.
