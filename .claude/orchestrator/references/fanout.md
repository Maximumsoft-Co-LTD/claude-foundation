...
- **On `inline-fallback`, reissue inline** — every spawn `subagent_type="general-purpose"` + the worker's role contract (`.claude/agents/team-<role>.md`) read into the prompt; parallelism preserved (one message, N calls); record actual `subagent_type` per spawn for the `Dispatched-as:` map.
