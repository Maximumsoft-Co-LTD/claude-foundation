...
1. **Nest one `general-purpose` helper per repo** — foreground, all in one message, each carrying the **Per-repo variant** contract inlined + `repo_root=<r>` + `model="sonnet"` (Case 6 requires the pin on a mid-run general-purpose spawn). Dispatch mechanics: **Common shape** above.
