...
- `skip <n>`/`run <n>` → discretionary only (5 Test / 6 Review / 8 Docs); refuse protected (1/2/3/7/10) or non-discretionary (4/9). Edit `plan.md` row + `state.json > phase_plan.<…>`. `skip 5` on feat/fix/refactor needs explicit waiver of the regression/baseline contract.
