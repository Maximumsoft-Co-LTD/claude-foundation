...
**One-batch gate (XS/S):** print the summary, then fold the loop's first round into ONE `AskUserQuestion` — Q1 `approve | revise` (the per-line AC confirm rides the summary block), Q2 `commit on ship?` (default no; a `yes` resolves the PR question in the same batch), Q3 only when the plan carries deviations (confirm each). A `revise` answer re-enters the normal loop. M/L keep the sequential rounds.

- `revise` → **targeted incremental edit, never full regeneration.** Plan-only → `lead` plan-revise (`.claude/agents/references/lead.md > Revise variant`), edits only affected sections. Requirements → `pm` spec-patch mode (edit affected sections; re-interview only if a new slot opened), then `lead` plan-revise + (feat/fix/refactor) `qa` test-plan-revise if an AC moved or a gap resolved. Test-plan-only → `qa` test-plan-revise. Re-present only changed parts; loop until `approve`. **Artifact shape rides `revise`:** the spec's Type block set is the shape-matrix default (`plan-writing > references/size-tiering.md > Artifact shape by Type`); a revise may ADD sections (additive deviation, noted in the artifact), never remove the contract core (`## Goal` / `**Type**:` / `AC#` / `T###`+`verify:`).
