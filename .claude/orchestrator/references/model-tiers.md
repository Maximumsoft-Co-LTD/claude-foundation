...
| `lead` | sonnet default, opus escalation | The one worker allowed to vary freely (guard Case 4 lets it). **The frontmatter pins the sonnet default**, so an omitted `model` param lands on sonnet; opus is passed explicitly. Escalation list: `references/lead.md > Model note`. Mode C (security — rides the review spawn when the trigger fires) always opus: a fired trigger makes the merged review+security spawn opus. |
| `engineer` | sonnet, **may escalate to opus** | Execution work guided by the plan — but `phase-2.md` op 5 escalates implement at `Size=L` or on a security-trigger path ("implement is where defects are born"). Case 4 allows that one upward override and still blocks a downgrade. |
